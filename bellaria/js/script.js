(function ($) {
  ...
  if ($("#email-form").length) {
    $("#submit").click(function (event) {
      ...
      emailjs
        .sendForm("service_pi1lzem", "template_7uup7tx", "#email-form")
        .then(
          function (response) {
            console.log("SUCCESS!", response.status, response.text);
            $("#email-form .response").html(
              '<div class="success">Your mail has been sent successfully!</div>'
            );
            $("#email-form").trigger("reset"); // Réinitialise le formulaire après l'envoi
          },
          function (error) {
            console.log("FAILED...", error);
            $("#email-form .response").html(
              '<div class="failed">Failed to send email. Please try again later.</div>'
            );
          }
        );
    });
